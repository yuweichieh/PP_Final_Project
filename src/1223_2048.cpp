...
public:
	...
	/*my version of select_best_move*/
	state select_best_move(const board& b) const {
		state after[4] = { 0, 1, 2, 3 }; // up, right, down, left
		state* best = after;
		debug << "choosing best move\n" ;
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {	// compute afterstate(s,a)
				debug << "move\n" << move->after_state() << std::endl ;
				board rem ;
				rem = move->after_state() ;	
				int n0 = 0 ;	// number of zeros
				for ( int i = 0; i < 16; i++ )	// count number of zero
					if ( rem.at(i) == 0 )
						n0++ ;
				double summing = 0.0 ;
				for ( int i = 0 ; i < 16 ; i++ ){
					if ( rem.at(i) != 0 )continue ;
					board temp;
					temp = rem ;
					temp.set(i, 1) ;
					double e = estimate(temp) ;
					summing += e * 0.9 / n0 ;
				}
				for ( int i = 0 ; i < 16 ; i++ ){
					if ( rem.at(i) != 0 )continue ;
				...
				move->set_value(move->reward() + summing) ;
				//info << move->reward() << "   #####   " << summing << std::endl ;
				if (move->value() > best->value())
					best = move;
			} else {
				move->set_value(-std::numeric_limits<float>::max());
	...
	*/
	/*my version of update_episode*/
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		float exact = 0;
		for (path.pop_back(); path.size(); path.pop_back()) {
			state& move = path.back();
			float error = exact - (estimate(move.before_state()) - move.reward());
			//info << exact << "\t" << move.value() << "\t" << move.reward() << std::endl ;
			debug << "update error = " << error << " for after state" << std::endl << move.before_state();
			double u = update(move.before_state(), alpha * error);
			//info << u << std::endl;
			exact = move.reward() + u;
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	if ( rank == 0 ){	// master
		std::vector<float> catstring;
		catstring.reserve(featsize*4+1);
/*
...
*/
		for(int i=0; i<tasks-1; i++){
			...
			MPI_Recv(&path_receive[0], round, obj_type, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
...
//			std::cout<< "chk point1 pass\n";
//			tdl.make_statistic()
			for(int i=0; i<4; i++){
//				std::cout<< "LEN"<<i<<": "<< tdl.feats[i]->length<< std::endl;
				for(int j=0; j<featsize; j++){
					catstring.push_back(tdl.feats[i]->weight[j]);
					std::cout << tdl.feats[i]->weight[j]<< std::endl;
				}
			}	
			MPI_Send(&catstring[0], featsize * 4, MPI_FLOAT, status.MPI_SOURCE, 0, MPI_COMM_WORLD);
		}
	}
	else{				// workers
		board b;
...
*/
		...
		MPI_Recv(&recv[0], featsize*4, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, &status);
		for(int i=0; i<featsize*4; i++){
