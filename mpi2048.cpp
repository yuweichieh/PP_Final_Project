	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			for (int t = 1, c = 0; c < unit; c += stat[t++]) {
				...
				progress << "\t" << ((1 << t) & -2u) << "\t" << (accu * coef) << "%";
				//if ( ((1 << t) & -2u) == 2048 )
				//	to_file << n << " " << (accu * coef ) << std::endl ;
				info << "\t(" << (stat[t] * coef) << "%)" << std::endl;
				progress << "\t(" << (stat[t] * coef) << "%)" << std::endl;
			}
			scores.clear();
			maxtile.clear();
		}
...
int main(int argc, const char* argv[]) {
	...
	*/		
			...
			if ( game_count % 1000 == 999 ){
				progress << "game_count = "<< game_count+1<< std::endl;
				...
				progress << "average gaming time  : " << total_time_game / 1000.0 << "ms" << std::endl ;
				total_time_game = 0.0;
			}
